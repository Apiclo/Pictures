...
<?php $defaults = array(
		...
		'label_submit'=>__('发送','mdx'),
		'comment_notes_after'=>'',
		'id_form'=>'commentform',
		'cancel_reply_link'=>__('取消回复', 'mdx'),
		'comment_field'=>'<div class="mdx-comment-main-input mdui-textfield mdui-textfield-floating-label'.$mdx_emj_cla.'"><i class="mdui-icon material-icons">textsms</i><label class="mdui-textfield-label">'.__('留下一句话再走吧','mdx').'</label><textarea class="mdui-textfield-input" name="comment" id="comment"></textarea></div>'.$mdx_emj_ele.$mdx_alu,
		'fields'=>apply_filters('comment_form_default_fields', array(
				'author'=>'<div class="mdui-textfield mdui-textfield-floating-label disfir disfirleft"><i class="mdui-icon material-icons">account_circle</i><label class="mdui-textfield-label">'.__('该怎么称呼您？','mdx').'</label><input class="mdui-textfield-input" type="text" id="author" name="author" value="'.esc_attr($comment_author).'" '.($req?"required":'').'><div class="mdui-textfield-error" role="alert">'.__('昵称不能为空', 'mdx').'</div></div>',

				'email'=>'<div class="mdui-textfield mdui-textfield-floating-label disfir disfirright"><i class="mdui-icon material-icons">email</i><label class="mdui-textfield-label">'.__('留下邮箱，方便以后联系','mdx').'</label><input class="mdui-textfield-input" type="email" id="email" name="email" value="'.esc_attr($comment_author_email).'" '.($req?"required":'').'><div class="mdui-textfield-error" role="alert">'.__('请按格式填写邮箱', 'mdx').'</div></div>',
